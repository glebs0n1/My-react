...
const Cats: React.FC = () => {
  ...
  const filteredPets = useMemo(() => {
    return pets.filter((pet) => {
      if (!pet.species?.toLowerCase().includes("kat")) return false;
  ...
  const uniqueValues = useMemo(() => {
    const catPets = pets.filter((p) => p.species === "Katės");

    const getUnique = (field: keyof Pet): string[] =>
      Array.from(
        new Set(catPets.map((p) => p[field]).filter(Boolean) as string[])
      ).sort();
  ...
  return (
    <div className="min-h-screen bg-gradient-to-b from-gray-50 to-white">
      ...
      <Donation
        ...
        text="Jūsų auka padeda mums rūpintis šunimis, kuriems reikia pagalbos."
        cta="Aukoti Dabar"
      />
    </div>
  );
